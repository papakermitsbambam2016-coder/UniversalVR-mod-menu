...
namespace ChainsawBONELABCodeMod
{
    public sealed class Main : MelonMod
    {
        ...
        private void OnGrab(GameObject objectToAttach, Hand hand)
        {
            ...
            MelonLogger.Msg("Chainsaw detected: " + chainsawRoot.name + ". Blades=" + state.BladeCount + ", BladeColliders=" + state.BladeColliderCount + ", MotorSounds=" + state.MotorSoundCount);
        }
        ...
        private static bool LooksLikeChainsawName(string value)
        {
            ...
            return value.IndexOf("chainsaw", StringComparison.OrdinalIgnoreCase) >= 0 || value.IndexOf("DoomHunterChainsaw", StringComparison.OrdinalIgnoreCase) >= 0 || value.IndexOf("Doom Hunter Chainsaw", StringComparison.OrdinalIgnoreCase) >= 0;
        }

        private static bool IsDoomHunterMarker(string value)
        {
            if (string.IsNullOrEmpty(value)) return false;
            return value.IndexOf("slashTop", StringComparison.OrdinalIgnoreCase) >= 0 || value.IndexOf("slashBottom", StringComparison.OrdinalIgnoreCase) >= 0 || value.IndexOf("Pull Cord", StringComparison.OrdinalIgnoreCase) >= 0 || value.IndexOf("StabPoint", StringComparison.OrdinalIgnoreCase) >= 0;
        }
        ...
        private static GameObject ResolveChainsawRoot(GameObject grabbed)
        {
            ...
            while (current != null)
            {
                if (LooksLikeChainsawName(current.name)) highestLikelyRoot = current.gameObject;
                current = current.parent;
            }
            if (highestLikelyRoot != null) return highestLikelyRoot;

            Transform searchRoot = grabbed.transform;
            while (searchRoot.parent != null && searchRoot.parent.parent != null) searchRoot = searchRoot.parent;
            Transform[] children = searchRoot.GetComponentsInChildren<Transform>(true);
            int doomMarkers = 0; bool hasBlade = false; bool hasGrip = false;
            foreach (Transform child in children)
            {
                if (child == null) continue;
                string n = child.name ?? string.Empty;
                if (IsDoomHunterMarker(n)) doomMarkers++;
                if (n.IndexOf("Blade", StringComparison.OrdinalIgnoreCase) >= 0 || n.IndexOf("slashTop", StringComparison.OrdinalIgnoreCase) >= 0 || n.IndexOf("slashBottom", StringComparison.OrdinalIgnoreCase) >= 0) hasBlade = true;
                if (n.IndexOf("Grip", StringComparison.OrdinalIgnoreCase) >= 0) hasGrip = true;
            }
            if (doomMarkers >= 2 && hasBlade && hasGrip) return searchRoot.gameObject;
            ...
            foreach (Transform child in grabbedChildren)
            {
                ...
                if (name.IndexOf("BladeTransform", StringComparison.OrdinalIgnoreCase) >= 0 || name.IndexOf("BladeCollider", StringComparison.OrdinalIgnoreCase) >= 0 || name.IndexOf("IdleSound", StringComparison.OrdinalIgnoreCase) >= 0) return grabbed;
            }
            return null;
        }
        ...
        private sealed class ChainsawRuntime
        {
            ...
            public ChainsawRuntime(GameObject rootObject)
            {
                ...
                foreach (Transform t in transforms)
                {
                    ...
                    if (n.IndexOf("BladeTransform", StringComparison.OrdinalIgnoreCase) >= 0 || string.Equals(n, "Blade", StringComparison.OrdinalIgnoreCase) || n.IndexOf("slashTop", StringComparison.OrdinalIgnoreCase) >= 0 || n.IndexOf("slashBottom", StringComparison.OrdinalIgnoreCase) >= 0) blades.Add(t);
                }
                ...
                foreach (AudioSource audio in audioSources)
                {
                    ...
                    if (n.IndexOf("IdleSound", StringComparison.OrdinalIgnoreCase) >= 0 || n.IndexOf("BladeAudio", StringComparison.OrdinalIgnoreCase) >= 0 || n.IndexOf("Motor", StringComparison.OrdinalIgnoreCase) >= 0 || n.IndexOf("Chainsaw", StringComparison.OrdinalIgnoreCase) >= 0 || n.IndexOf("Idle", StringComparison.OrdinalIgnoreCase) >= 0) motorSounds.Add(audio);
                }
            }
            ...
            {
                running = true;
                if (!Config.MotorSound) return;
                foreach (AudioSource audio in motorSounds)
                {
                    try { audio.loop = true; if (audio.clip != null && !audio.isPlaying) audio.Play(); }
                    catch (Exception e) { MelonLogger.Warning("Chainsaw audio start failed: " + e.Message); }
                }
            }
            ...
            public void Stop()
            {
                ...
                foreach (AudioSource audio in motorSounds)
                {
                    try { if (audio != null && audio.isPlaying) audio.Stop(); } catch { }
                }
            }
            ...
            public void Tick(float dt)
            {
                ...
                foreach (Transform blade in blades) if (blade != null) blade.Rotate(Vector3.forward, Config.BladeDegreesPerSecond * dt, Space.Self);
            }
            ...
            public void DamageNearby()
            {
                ...
                foreach (Collider collider in bladeColliders)
                {
                    ...
                    foreach (Collider hit in hits)
                    {
                        ...
                        if (brain == null || !damagedBrains.Add(brain)) continue;
                        try { brain.DealDamage(Config.Damage); }
